...
surv_loc_df=read.table("PhD_4th_yr/Spatial_var_inbreeding/first_year_survival_loc.txt", sep = ",", header = TRUE)
...
suv_model_simple=glmmTMB(first_year_survival~ Sex + MotherStatus + mum_age+mum_age_sq+Day_seq+FROH+
...
surv_yearling_reg=reg_pred%>%
  ...
  labs(x="Spatial region", y="Predicted yearling survival probability")+
