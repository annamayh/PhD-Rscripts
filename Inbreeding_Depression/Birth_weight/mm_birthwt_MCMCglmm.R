...
##sort out pedigree
ids_in<-as.matrix(birth_wt_df_na_rm%>%dplyr::select(Code))
...
birth_wt_model<-MCMCglmm(CaptureWt~1 + Sex + AgeHrs + MotherStatus + mum_age+mum_age_sq+FROHsum, 
                random= ~ Code +
...
save(birth_wt_model, file="PhD_4th_yr/Inbreeding_depression_models/birth_weight/birth_wt_model_output_incmumage_50kit_50bin.RData")
